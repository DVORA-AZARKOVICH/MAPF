...
def build_constraint_table(constraints, agent):
    ...
    for constraint in constraints:
        ...
        elif constraint.get('positive', False):
            other_agent_constraint = constraint.copy()
            other_agent_constraint['agent'] = agent 
            other_agent_constraint['positive'] = False 
            
            timestep = constraint['timestep']
            if timestep not in constraint_table:
                constraint_table[timestep] = []
            constraint_table[timestep].append(other_agent_constraint)
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    matching = [t for t, clist in constraint_table.items()
                if any(constraint['loc'] == [goal_loc] for constraint in clist)]
    earliest_goal_timestep = 0
    if matching:
        earliest_goal_timestep = max(matching) + 1
    ...
    while len(open_list) > 0:
        ...
        if curr['timestep'] > (len(my_map) * len(my_map[0])) ** 2:
            return None
